-- Migration: Exclude test tournaments from combo leaderboards

-- 1. Aggiorna combo_points_leaderboard per escludere i tornei di test
...
RETURNS TABLE(
  ...
  WITH all_rounds AS (
    ...
    WHERE r.winner_side IS NOT NULL
      AND (t.structure->>'is_test' IS NULL OR t.structure->>'is_test' <> 'true')
  ),
  all_entries AS (
...
-- 2. Aggiorna all_combos_leaderboard per escludere i tornei di test
