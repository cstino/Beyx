...
export function TournamentSetup({ onConfirm }) {
  ...
  const [assignmentMode, setAssignmentMode] = useState("random"); // 'random' | 'draft' | 'asta' | 'a_buste'
  ...
  const canStart =
    entryMode === "open" ? maxParticipants >= 2 : participants.length >= 2;
  ...
  return (
    <div className="space-y-8 pb-32">
      ...
      <div className="p-5 rounded-3xl bg-[#12122A] border border-white/5 focus-within:border-primary/20 transition-all">
        <div className="text-[10px] font-black text-primary tracking-[0.2em] mb-2 uppercase text-center">
          NOME TORNEO
        </div>
        <input
          type="text"
          ...
          placeholder={
            entryMode === "open"
              ? "Nome del Torneo Pubblico..."
              : "es. Torneo d'Estate 2024"
          }
            ...
            <div className="p-4 rounded-2xl bg-[#12122A] border border-white/10 relative overflow-hidden flex flex-col justify-between">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-black text-white uppercase font-createfuture italic">
                    Bey Titolari
                  </span>
                </div>
              ...
              <div className="flex items-center justify-between gap-3 mt-4 bg-white/5 p-2 rounded-xl border border-white/5">
                <button
                  onClick={() => {
                    ...
                    setBattleType(
                      r > 0
                        ? `${starterBeysCount}v${starterBeysCount} (+${r})`
                        : `${starterBeysCount}v${starterBeysCount}`,
                    );
                  }}
                ...
                <span
                  className={`text-2xl font-black font-createfuture italic tabular-nums ${reserveBeysCount ? "text-[#4361EE]" : "text-white/20"}`}
                >
                  {reserveBeysCount}
                </span>
                <button
        ...
        {winCondition === "point_target" ? (
          ...
          >
            ...
            <p className="text-[8px] text-white/20 font-bold uppercase tracking-widest text-center px-4">
              Il primo blader che raggiunge {pointTarget} punti vince
              l'incontro.
            </p>
          </motion.div>
        ) : (
          ...
          >
            <div className="text-[10px] font-black text-[#4361EE] tracking-widest uppercase italic text-center">
              Modalità Total Battle
            </div>
            <p className="text-[9px] text-white/40 font-medium text-center leading-relaxed">
              I blader dovranno disputare tutti i {starterBeysCount} Round
              previsti.
              <br />
              Vince chi accumula più punti totali. In caso di parità, viene
      ...
      {format === "round_robin" && (
        ...
        >
          <div className="space-y-3">
            <label className="text-[9px] font-black text-primary tracking-[0.2em] uppercase px-1">
              Giri del Girone (A/R)
            </label>
            <div className="flex items-center gap-4 bg-[#0A0A1A] rounded-2xl p-2 border border-white/5">
              <button
                onClick={() => setRrCycles(Math.max(1, rrCycles - 1))}
                className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-white"
              >
                <Minus size={16} />
              </button>
              <div className="flex-1 text-center font-black text-xl text-white italic">
                {rrCycles} {rrCycles === 1 ? "Giro" : "Giri"}
              </div>
              <button
                onClick={() => setRrCycles(rrCycles + 1)}
                className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-white"
              >
                <Plus size={16} />
              </button>
            ...
            <p className="text-[8px] text-white/20 font-bold uppercase tracking-widest text-center">
              {rrCycles === 1
                ? "Ogni blader sfida gli altri una volta."
                : `Ogni blader sfida gli altri ${rrCycles} volte.`}
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex bg-[#0A0A1A] rounded-2xl p-1 border border-white/5">
              <button
                onClick={() => setRrWinnerMode("points")}
                className={`flex-1 py-3 rounded-xl text-[10px] font-black tracking-widest transition-all ${rrWinnerMode === "points" ? "bg-[#4361EE] text-white" : "text-white/30"}`}
              >
                PUNTI
              </button>
              <button
                onClick={() => setRrWinnerMode("playoff")}
                className={`flex-1 py-3 rounded-xl text-[10px] font-black tracking-widest transition-all ${rrWinnerMode === "playoff" ? "bg-[#4361EE] text-white" : "text-white/30"}`}
              >
                PLAYOFF
              </button>
            </div>
          ...
          {rrWinnerMode === "playoff" && (
            <motion.div
              ...
              className="space-y-3"
            >
              <label className="text-[9px] font-black text-[#4361EE] tracking-[0.2em] uppercase px-1">
                Tipo di Playoff
              </label>
                ...
                {[
                  {
                    id: "final",
                    label: "FINALE (TOP 2)",
                    desc: "Il primo e il secondo si sfidano in finale.",
                  },
                  {
                    id: "semi",
                    label: "SEMIFINALI (TOP 4)",
                    desc: "1° vs 4° e 2° vs 3°. I vincenti vanno in finale.",
                  },
                  {
                    id: "play_in",
                    label: "PLAY-IN (TOP 6)",
                    desc: "1° e 2° aspettano. 3°vs6° e 4°vs5° per le semi.",
                  },
                ].map((t) => (
                  <button
                    key={t.id}
                  ...
                  >
                    <div
                      className={`text-[10px] font-black uppercase italic ${playoffType === t.id ? "text-[#4361EE]" : "text-white/40"}`}
                    >
                      {t.label}
                    </div>
                    <div className="text-[8px] font-bold text-white/20 uppercase mt-0.5">
                      {t.desc}
                    </div>
                  </button>
                ))}
          ...
          {beybladeMode === "pool" ? (
            ...
            >
              <div className="p-5 rounded-[32px] bg-primary/5 border border-primary/10 space-y-4">
                <label className="text-[8px] font-black text-primary tracking-[0.2em] uppercase block text-center">
                  Metodo di Assegnazione Pool
                </label>
                <div className="grid grid-cols-4 gap-2">
                  {[
                    ...
                    >
                      <span
                        className={`text-[9px] font-black uppercase tracking-tighter ${assignmentMode === m.id ? "text-primary" : "text-white/40"}`}
                      >
                        {m.label}
                      </span>
                    </button>
                ...
                <div className="bg-black/20 p-3 rounded-2xl">
                  <p className="text-[8px] text-white/40 font-medium leading-relaxed text-center uppercase tracking-wider">
                    {assignmentMode === "random" &&
                      "I Beyblade verranno assegnati ai partecipanti in modo completamente casuale."}
                    {assignmentMode === "draft" &&
                      "I partecipanti sceglieranno i Beyblade seguendo un ordine a serpentina."}
                    {assignmentMode === "asta" &&
                      "Ogni giocatore riceve 50 crediti per Bey da acquistare. Base d'asta: 1 credito."}
                    {assignmentMode === "a_buste" &&
                      "Ogni giocatore riceve 50 crediti per Bey. Le offerte sono segrete: vince l'offerta più alta (a parità, la prima inviata)."}
                  </p>
                </div>
              </div>
            </motion.div>
            ...
            >
              Ogni partecipante utilizzerà i propri Beyblade salvati nel
              profilo.
            </motion.p>
          )}
        </AnimatePresence>
        ...
        {entryMode === "invitation" ? (
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="text-[11px] font-black text-white tracking-[0.2em] uppercase pl-1">
                Invitati ({participants.length})
              </h3>
              {participants.length < 2 && (
                <div className="text-[9px] font-bold text-white/20 italic animate-pulse">
                  Invita almeno 2 Blader
                </div>
              )}
            </div>
              ...
              {participants.map((p, i) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center font-black text-[10px] text-white/40">
                      {i + 1}
                    </div>
                  ...
                  <button
                    ...
                    className="p-2 text-white/10 hover:text-primary"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
            ...
            <div className="pt-2 space-y-4">
              <div className="flex gap-2">
                ...
                <button
                  onClick={addGuest}
                  className="w-14 h-14 rounded-2xl bg-[#4361EE] flex items-center justify-center text-white active:scale-95 shadow-lg shadow-[#4361EE]/20"
                >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-white/70 uppercase tracking-wider truncate">
                          Seleziona Blader Registrati ({availableUsers.length}{" "}
                          disponibili)
                        </span>
                      </div>
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <div className="p-5 rounded-3xl bg-[#12122A] border border-white/5">
                <div className="text-[9px] font-black text-white/30 tracking-widest uppercase mb-2">
                  Max Blader
                </div>
                <input
                  type="number"
              ...
              <div className="p-5 rounded-3xl bg-[#12122A] border border-white/5 flex flex-col justify-center">
                <div className="text-[9px] font-black text-white/30 tracking-widest uppercase mb-1">
                  Visibilità
                </div>
                <div className="text-sm font-black text-primary uppercase italic">
                  Public Arena
                </div>
              </div>
            </div>
      ...
      <motion.button
        onClick={() =>
          onConfirm({
            ...
            playoffType:
              format === "round_robin" && rrWinnerMode === "playoff"
                ? playoffType
                : null,
            beybladeMode,
        ...
        style={{
          ...
          boxShadow: canStart
            ? "0 10px 40px -10px rgba(233,69,96,0.5)"
            : "none",
        }}
      >
        <div className="relative z-10 flex items-center gap-3">
          {entryMode === "open" ? "APRI ISCRIZIONI" : "CREA E INVITA"}{" "}
