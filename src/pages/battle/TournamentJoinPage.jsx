...
export default function TournamentJoinPage() {
  ...
  async function checkExistingRegistration() {
    ...
    if (data) {
      setExistingReg(data);
      // Se siamo in modalità pool, l'iscrizione è completa anche senza deck
      // Altrimenti serve il deck_config
      if (tournament?.beyblade_mode === 'pool' || data.deck_config) {
        setDone(true);
      }
    }
  }
    ...
    return (
      <div className="min-h-screen bg-[#0A0A1A] flex flex-col items-center justify-center p-6 text-center">
        ...
        <div className="flex flex-col gap-3 mt-10">
          {isApproved && (
            <button 
              onClick={() => navigate(`/battle/tournament/${id}`)} 
              className="px-8 py-4 bg-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-glow-primary"
            >
              Vai alla Pagina del Torneo
            </button>
          )}
          <button onClick={() => navigate('/battle')} className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black text-white uppercase tracking-[0.2em]">Torna all'Arena</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0A0A1A] pb-32 pt-4">
      ...
      <div className="px-6 space-y-8">
        <div className="p-5 rounded-3xl bg-primary/5 border border-primary/20 flex items-start gap-4">
           ...
           <p className="text-xs text-white/60 font-medium leading-relaxed">
             {tournament?.beyblade_mode === 'pool' 
               ? 'In questo torneo i Beyblade verranno assegnati casualmente da un pool. Non devi selezionare il tuo deck ora.'
               : (tournament?.description || 'Nessuna descrizione particolare fornita.')}
           </p>
        </div>
        ...
        <button 
          ...
          disabled={submitting || (tournament?.beyblade_mode !== 'pool' && deck.some(b => !b.blade_id))}
